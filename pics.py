...
with open("C:/Users/charl/Downloads/WORD2VEC/harry_potter_extracted/harry_potter/HP1.txt", "r", encoding="utf-8", errors="ignore") as f:
    text = f.read().lower()
...
print(f"Total tokens before filtering rare words: {len(tokens_all)}")
...
print(f"Total tokens after filtering rare words (min_count={min_count}): {len(tokens_all)}")
print("Sample tokens:", tokens_all[:20])
...
print(f"Train tokens count: {len(tokens_train)}")
print(f"Test tokens count: {len(tokens_test)}")
...
print("First 20 words in vocab:", idx2word[:20])
...
print("First 10 training pairs (center_idx, context_idx):", pairs[:10])
...
for epoch in range(epochs):
    ...
    for batch_idx, (centers, contexts) in enumerate(dataloader):
        ...
        if batch_idx % 100 == 0:
            print(f"Epoch {epoch+1}, Batch {batch_idx}, Batch Loss: {loss.item():.4f}")
    print(f"Epoch {epoch+1} - Total Loss: {total_loss:.4f}")

# Print some embeddings for first 5 words
print("\nSample embeddings for first 5 words:")
for w in idx2word[:5]:
    idx = word2idx[w]
    emb_vector = model.emb.weight[idx].detach().cpu().numpy()
    print(f"Word: '{w}' - Embedding vector (first 5 dims): {emb_vector[:5]}")

# --------- 7. POS tagging ---------
# Tag all tokens (train+test)
print("\nStarting POS tagging for all tokens...")
tagged_all = pos_tag(tokens_all)
tagged_all = [(w, map_tag('en-ptb', 'universal', t)) for (w, t) in tagged_all]
print(f"Total tokens tagged: {len(tagged_all)}")
...
print(f"Unique POS tags: {tagset}")
print(f"Total words with embeddings & tags: {len(y)}")
...
print(f"POS Probe Train size: {len(train_idx)}, Test size: {len(test_idx)}")

# --------- 8. Baseline accuracy ---------
major_class = Counter(y_train.tolist()).most_common(1)[0][0]
baseline_acc = (y_test == major_class).float().mean().item()
print(f"Baseline accuracy (most frequent POS tag): {baseline_acc:.4f}")
...
for epoch in range(probe_epochs):
    ...
    print(f"Probe Epoch {epoch+1} - Loss: {loss.item():.4f}")
...
for i in range(len(words_test)):
    ...
    if len(word) >= 4:
        ...
        count += 1
        if count >= 20:  # limit output to 20 examples
            break
